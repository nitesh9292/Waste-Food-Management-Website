                ...
                <div class="cd-full-width">       
                    <div class="container">
                       <div class="col-md-11 ">
                         ...
                         <div>
                         <div>
                         <h2 style="color:red;"><center>Food Requests from NGOs</center></h2>
                        </div>
<?php
...
//$sql = "SELECT id,(3959 * acos( cos( radians( $v1 )) * cos( radian( lat )) * cos( radians( lng ) - radians($v2)
  ...
  $sql = "SELECT * FROM food_requests WHERE donor_email = '{$_SESSION['email']}' AND status = 'pending'";
...
<table class="rwd-table">
  <tr>
    ...
    <th><center>NGO Contact No</center></th>
    <th><center>NGO Email</center></th>
...
while ($row = mysqli_fetch_assoc($result)) {
              ...
              ?>
                         <tr>
                             <td data-th="Food id">            <?php echo "<h7>".$row['food_id']."</h7>"; ?>  
                              <td data-th="Food Image" class="img-responsive">     <?php echo "<img width='200' height='200' src='uploaded images/".$row['image']."'  >"; ?>               </td>
                              
                              <td  data-th="Description">    <?php echo "<h7>".$row['food_name']."</h7>"; ?>                             </td>
                              <td data-th="NGO Name">            <?php echo "<h7>".$row['ngo_name']."</h7>"; ?>                                    </td>
                              <td data-th="NGO Contact no">      <?php echo "<h7>".$row['ngo_contact']."</h7>"; ?>                                 </td>
                              <td data-th="NGO email">            <?php echo "<h7>".$row['ngo_email']."</h7>"; ?>  
                              <!-- <td data-th="status">            <?php //echo "<h7>".$row['status']."</h7>"; ?>   -->
                              <td><center><a href="delete.php?id=<?php echo $row['food_id'];?>" style="background-color:red; text-decoration:none; border:3px solid black; color:black; font-weight:bold;">Deliver</a></center></td>
                              <!-- <td><center><form action="delete.php" method="POST">
                              <input type="hidden" name="food_id" value="<?php echo $row['food_id']; ?>">
                                <button type="submit"  name="delete_row"  class=" btn btn-success ">Delivered</button></form></center></td> -->
                              
                              <!-- <td><center> <form action="respond.php" method="post">
             <input type="hidden" name="request_id" value="<?php// echo $row['food_id']; ?>">
            <input type="hidden" name="donor_email" value="<?php// echo $_SESSION['email']; ?>">
            <input type="hidden" name="ngo_email" value="<?php //echo $row['ngo_email']; ?>">
